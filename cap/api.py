...
def create_api_app() -> FastAPI:
    ...
    app = FastAPI(
        title="Commercial Analytical Platform (CAP) API",
    ...
    async def root():
        """Root endpoint."""
        return {
            "message": "Commercial Analytical Platform (CAP) API", 
            "version": "1.0.0",
    ...
    async def health_check():
        ...
        return {"status": "healthy", "service": "cap"}
    ...
    async def calculate_metric_csv(metric_id: str, **params):
        ...
        try:
            ...
            if isinstance(result, pd.DataFrame) or (pl is not None and isinstance(result, pl.DataFrame)):
...
def _process_result(result: Any, output_format: str = "json") -> tuple[str, Any]:
    ...
    if isinstance(result, pd.DataFrame) or (pl is not None and isinstance(result, pl.DataFrame)):
        ...
        for key, value in result.items():
            if isinstance(value, pd.DataFrame) or (pl is not None and isinstance(value, pl.DataFrame)):
