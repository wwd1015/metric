...
class MetricRegistry:
    ...
    def __init__(self, metrics_module: str = "cap.metrics"):
    ...
    def load_metrics(self):
        ...
        missing_functions = set(self.metrics.keys()) - set(self._functions.keys())
        if missing_functions:
            logger.warning(
                "Configurations found without registered metric functions: %s",
                ", ".join(sorted(missing_functions)),
            )

        orphan_functions = set(self._functions.keys()) - set(self.metrics.keys())
        if orphan_functions:
            logger.warning(
                "Metric functions registered without configuration files: %s",
                ", ".join(sorted(orphan_functions)),
            )
        ...
        try:
            ...
            for py_file in metrics_path.glob("**/*.py"):
                ...
                try:
                    ...
                    # Import or reload the module to pick up recent changes
